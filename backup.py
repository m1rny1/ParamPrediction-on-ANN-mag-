...
def import_data(path=path, sheet=sheet, cols=[0,1]):
  df = pd.read_excel(path, sheet_name=sheet , header = 0, usecols=cols)
  ...
  return df, plot_cols
df, plot_cols =import_data(path, sheet, [2,3])
print(df.shape)
...
multi_window.plot(model, plot_cols)                                                     # побудова графіка отриманих результатів
plt.show()

model.save('convAlCold.h5')
