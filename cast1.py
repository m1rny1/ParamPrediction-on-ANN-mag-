...
# Транспонування матриці та виділення числових даних з Pandas.DataFrame до NumPy.Array
# df = dfr.transpose()
# df = df.to_numpy()  
...
plot_cols = ['PEMAG 1', 'PEMAG 2', 'PEMAG 3', 'PEMAG 4', 'PEMAG 5']
plot_features = df[plot_cols]
plot_features.index = df['Час, с']
_ = plot_features.plot(subplots=True)
timer.start()
plt.show()

# побудова зрізу даних, обмеженого 480 елементами
# plot_features = df[plot_cols][:480]
# plot_features.index = date_time[:480]
# _ = plot_features.plot(subplots=True, title="Start dataset until #480")
# timer.start()
# plt.show()
...
class WindowGenerator():
  ...
  def plot(self, model=None, plot_col=plot_cols[0], max_subplots=5):
...
OUT_STEPS = 4    
multi_window = WindowGenerator(input_width=6,            # 24 вхідних кроки
...
for i in range(len(plot_cols)):
    multi_window.plot(plot_col=plot_cols[0])   # побудова графіку вікна даних
    plt.show()
...
multi_window.plot(multi_conv_model)                                                     # побудова графіка отриманих результатів
# timer.start()
plt.show()
